...
/**
 * Welcome Interactor Transformer.
 *
 * @version 1.0 Apr 01 2017
 * @author  downleader
 */
public class WelcomeInteractorTransformer implements ViperTransformer<WelcomeInteractorState, Parcelable> {
    
    @Override
    public OuterState exportState(WelcomeInteractorState inner) {
        return new OuterState(inner.getValue());
    }
    
    @Override
    public WelcomeInteractorState importState(Parcelable outer) {
        OuterState outerState = (OuterState) outer;
        return new WelcomeInteractorState(outerState.getValue());
    }
