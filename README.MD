...
```
`PresenterImpl` class needs to implement `Presenter` interface and extend `AbstractPresenter` abstract class, parameterized with `Repository` interface, the expected `Repository.Presenter` interface and the expected `Presenter.View` interface.
...
The requirements described above can be satisfied by `Activity` only. That's why it needs to call:
...
Default implementation of all the VPER components is stateless, so there is no need to call any methods to save or restore it.

## Examples
In the project's code there are two VPER modules implemented: `welcome` and `goodbye`. If you wanna go hardcore and maintain a separate state for each of the components, go and learn how `welcome` module is implemented. If you just want to code already, jump into `goodbye` module and see how easy and safe it is to use VPER in Android.
